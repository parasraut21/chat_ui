...
export default function ChatContainer({ currentChat, socket, contacts }) {
  ...
  useEffect(async () => {
    ...
    const response = await axios.post("https://chat-server2-ab56.onrender.com/api/messages/getmsg", {
      from: data._id,
      to: currentChat._id,
    });
  ...
  const handleSendMsg = async (msg) => {
    ...
    await axios.post("https://chat-server2-ab56.onrender.com/api/messages/addmsg", {
      from: data._id,
      to: currentChat._id,
      message: msg,
    });
  ...
  useEffect(() => {
    if (socket.current) {
      ...
      socket.current.on("typing..", (data) => {
        console.log("data id", data._id ,"--from",data.from);
        ...
        if (data._id === localData._id && data.from !== localData._id) {
